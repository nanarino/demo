...
def category_commit(org_db,
    ...
    org_db.execute(f"""insert into ele_shop_cate 
                (shop_id, category_id, E_catname, E_catcode, parent_category_id)
            VALUES
                ({repr(shop_id)}, {category_id}, {repr(E_catname)}, {repr(E_catcode)}, {parent_category_id})
            """)
    org_db.commit()
